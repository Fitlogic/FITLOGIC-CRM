      ...
      {forms.map((form) => (
        <Card
          ...
          className={`cursor-pointer transition-all hover:shadow-elevated ${
            selectedId === form.id ? "ring-2 ring-primary" : ""
          }`}
            ...
            <div className="flex items-start justify-between">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <Badge variant={form.active ? "default" : "secondary"} className="text-[10px] shrink-0">
...
function FormBuilder({
  ...
  form: IntakeForm;
  onUpdate: (id: string, updates: Partial<IntakeForm>) => void;
}) {
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <Select
                  ...
                  onValueChange={(v) => updateQuestion(selectedQuestion.id, { type: v as QuestionType })}
                >
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => updateQuestion(selectedQuestion.id, { helpText: e.target.value || undefined })}
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => updateQuestion(selectedQuestion.id, { placeholder: e.target.value || undefined })}
                  className="mt-1"
                />
              </div>
...
// ── Form Preview Dialog ──
function FormPreview({ form, open, onClose }: { form: IntakeForm; open: boolean; onClose: () => void }) {
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[85vh]">
        <DialogHeader>
          <DialogTitle className="font-heading">{form.name}</DialogTitle>
...
function SubmissionsPanel({
  ...
  const filtered = submissions.filter((sub) => {
    const matchSearch =
      sub.patientName.toLowerCase().includes(search.toLowerCase()) ||
      sub.patientEmail.toLowerCase().includes(search.toLowerCase());
    const matchReview = filterReview === "all" || sub.reviewStatus === filterReview;
    ...
    <div className="flex h-full">
      ...
      <div className="w-[420px] border-r flex flex-col">
        ...
        <ScrollArea className="flex-1">
          <div className="p-2 space-y-1">
            {filtered.map((sub) => {
              const reviewCfg = REVIEW_STATUS_CONFIG[sub.reviewStatus];
              const completionCfg = SUBMISSION_STATUS_CONFIG[sub.completionStatus];
              return (
                <div
                  key={sub.id}
                  className={`p-3 rounded-lg cursor-pointer transition-colors ${
                    selectedSub?.id === sub.id ? "bg-accent" : "hover:bg-muted"
                  }`}
                  onClick={() => setSelectedSub(sub)}
                >
                  <div className="flex items-start justify-between mb-1">
                    <span className="font-medium text-sm text-foreground">{sub.patientName}</span>
                    <Badge variant="outline" className={`${reviewCfg.bgColor} ${reviewCfg.color} text-[10px]`}>
                      {reviewCfg.label}
                    </Badge>
                  </div>
                  <p className="text-xs text-muted-foreground">{getFormName(sub.formId)}</p>
                  <div className="flex items-center gap-2 mt-1.5">
                    <Badge variant="outline" className={`${completionCfg.bgColor} ${completionCfg.color} text-[10px]`}>
                      {completionCfg.label}
                    </Badge>
                    {sub.submittedAt && (
            ...
            {filtered.length === 0 && (
              <p className="text-center text-sm text-muted-foreground py-8">No submissions found</p>
            )}
          </div>
        </ScrollArea>
      </div>
        ...
        {selectedSub ? (
          <ScrollArea className="h-full">
            <div className="p-6 space-y-6">
              <div className="flex items-start justify-between">
                <div>
                  <h2 className="font-heading text-lg font-bold text-foreground">{selectedSub.patientName}</h2>
                  <p className="text-sm text-muted-foreground">{selectedSub.patientEmail}</p>
                  <p className="text-xs text-muted-foreground mt-1">{getFormName(selectedSub.formId)}</p>
                </div>
                <div className="flex gap-2">
                  <Button
                    ...
                    onClick={() => onUpdateSubmission(selectedSub.id, { reviewStatus: "approved" })}
                  >
                  ...
                  <Button
                    ...
                    onClick={() => onUpdateSubmission(selectedSub.id, { reviewStatus: "needs_revision" })}
                  >
              ...
              <div className="space-y-4">
                ...
                {getFormQuestions(selectedSub.formId).map((q) => {
                  const answer = selectedSub.submissionData[q.id];
              ...
              <div className="space-y-2">
                ...
                <Textarea
                  value={selectedSub.staffNotes || ""}
                  onChange={(e) => onUpdateSubmission(selectedSub.id, { staffNotes: e.target.value || null })}
...
// ── Main Page ──
const IntakeForms = () => {
  const [forms, setForms] = useState<IntakeForm[]>(INTAKE_FORMS);
  ...
  const handleFormUpdate = (id: string, updates: Partial<IntakeForm>) => {
  ...
  const handleSubmissionUpdate = (id: string, updates: Partial<FormSubmission>) => {
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              {forms.length} templates · {totalSubmissions} submissions · {pendingCount} pending review
            </p>
          </div>
        </div>
      </div>
        ...
        <TabsContent value="templates" className="flex-1 min-h-0 mt-0">
          <div className="flex h-full">
            ...
            <div className="w-[340px] border-r flex flex-col">
              <div className="p-4 border-b">
                <Button onClick={handleAddForm} className="w-full gradient-brand text-primary-foreground">
              ...
              <ScrollArea className="flex-1">
                <div className="p-3">
                  <FormTemplates forms={forms} onSelect={setSelectedFormId} selectedId={selectedFormId} />
                </div>
              </ScrollArea>
            </div>
              ...
              {selectedForm ? (
                <>
                    ...
                    <div className="flex items-center gap-2">
                      <div className="flex items-center gap-2 mr-3">
                        <Switch
                          ...
                          onCheckedChange={(v) => handleFormUpdate(selectedForm.id, { active: v })}
                        />
                        <Label className="text-xs">{selectedForm.active ? "Active" : "Inactive"}</Label>
                      </div>
                      <Button size="sm" variant="outline" onClick={() => setPreviewForm(selectedForm)}>
                  ...
                  <div className="flex-1 min-h-0">
                    <FormBuilder form={selectedForm} onUpdate={handleFormUpdate} />
                  </div>
                </>
              ) : (
                <div className="flex flex-col items-center justify-center h-full text-muted-foreground">
